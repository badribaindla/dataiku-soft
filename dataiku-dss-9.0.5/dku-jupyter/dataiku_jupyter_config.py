...
# Default for NotebookApp is u"%(color)s[%(levelname)1.1s %(asctime)s.%(msecs).03d %(name)s]%(end_color)s %(message)s"
c.NotebookApp.log_format = '%(color)s[%(asctime)s.%(msecs).03d] [%(process)s/%(threadName)s] [%(levelname)s] [%(name)s]%(end_color)s %(message)s'
...
#import notebook.dataiku.kernelspecmanager
#c.NotebookApp.kernel_spec_manager_class = notebook.dataiku.kernelspecmanager.DataikuKernelSpecManager
...
import notebook.dataiku.dkucontentsmanager
c.NotebookApp.contents_manager_class = notebook.dataiku.dkucontentsmanager.DataikuContentsManager
...
import notebook.dataiku.sessionmanager
c.NotebookApp.session_manager_class = notebook.dataiku.sessionmanager.DataikuSessionManager

if hasattr(c, "LabApp"):
	c.LabApp.session_manager_class = notebook.dataiku.sessionmanager.DataikuSessionManager
...
# DKU: Force an invalid hash so that Jupyter does not try token-auth
c.NotebookApp.password = u'invalid_hash'
...
import notebook.dataiku.security
c.NotebookApp.login_handler_class = notebook.dataiku.security.DataikuJupyterSecurity

if hasattr(c, "LabApp"):
	c.LabApp.login_handler_class = notebook.dataiku.security.DataikuJupyterSecurity
...
import notebook.dataiku.kernelmanager
c.MultiKernelManager.kernel_manager_class = "notebook.dataiku.kernelmanager.DataikuIOLoopKernelManager"
...
#
import notebook.dataiku.dkucheckpoints
c.ContentsManager.checkpoints_class = notebook.dataiku.dkucheckpoints.DataikuCheckpoints
...
#import os
#c.DataikuKernelSpecManager.kernel_dirs = [
#	os.getenv("DKUINSTALLDIR") + "/kernelspecs"
#]
