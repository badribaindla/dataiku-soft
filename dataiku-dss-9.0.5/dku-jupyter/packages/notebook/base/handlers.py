...
class AuthenticatedHandler(web.RequestHandler):
    ...
    # Begin DKU specific
    # verify that the user has rights to access requested files and prevent dir traversal attacks (aka ../)
    def __init__(self, application, request, **kwargs):
        # enrich the handler with dku specific info such as list of allowed projects and is_admin
        # these two parameters are set in notebook.dataiku.security.DataikuJupyterSecurity.get_user()
        self.dku_allowed_projects = []
        self.dku_is_admin = False
        # call to super need to be after because get_user is called in it
        super(AuthenticatedHandler, self).__init__(application, request, **kwargs)

    def verify_path(self, path):
        """Raises a HTTPError(403) if not authorized"""
        if path is None or self.dku_is_admin is True:
            return
        split_path = path.strip('/').split('/')
        if len(split_path) == 0:
            self.log.info("Empty path not allowed")
            raise web.HTTPError(403)
        project_key = split_path[0]
        if project_key not in self.dku_allowed_projects:
            self.log.info("User %s not allowed to access project %s" % (self.current_user, project_key))
            raise web.HTTPError(403)
        for name in split_path:
            if name in ["..", "."]:
                self.log.info("User %s not allowed to access file at %s" % (self.current_user, path.strip('/')))
                raise web.HTTPError(403)
        self.log.info("Access allowed to project %s for user %s" % (project_key, self.current_user))


    def prepare(self):
        if "path" in self.path_kwargs:
            self.verify_path(self.path_kwargs["path"])
        if self.request.method in ["POST", "PATCH"] and self.request.uri.startswith(
                "/jupyter/api/contents") and self.request.body:
            data = json.loads(self.request.body)
            if "path" in data:
                self.verify_path(data["path"])
            if "copy_from" in data:
                self.verify_path(data["copy_from"])
...
class IPythonHandler(AuthenticatedHandler):
    ...
    def check_xsrf_cookie(self):
        ...
        # DKU SPECIFIC: DELETE session from backend is allowed
        if self.request.path.startswith("/jupyter/api/sessions") and self.request.method == "DELETE":
            return
...
class AuthenticatedFileHandler(IPythonHandler, web.StaticFileHandler):
    ...
    def get(self, path):
        ...
        # Begin DKU specific change the download path to get file from DSS git folder.

        # The $path variable is a relative path that locate the file, starting from inside the ipython_notebooks folder.
        # It should looks like: $PROJECT/$FOLDER/$NOTEBOOK_FILE
        # We to transform it to match the real filesystem.
        # We want to obtain: ../projects/$PROJECT/ipython_notebooks/$FOLDER/$NOTEBOOK_FILE

        # Separate the part of the path
        parts = path.strip(os.path.sep).split(os.path.sep)
        parts = [p for p in parts if p != ''] # remove duplicate splits
        # Inserting ipython_notebooks after the $PROJECT
        parts.insert(1, "ipython_notebooks")
        # Manually add the rest of the path
        path = os.path.join("..", "projects", *parts)
        # End DKU specific
    ...
    def validate_absolute_path(self, root, absolute_path):
        ...
        # Begin DKU specific: no longer check if the path is under the git root.
        # The right relative to which DSS folder was chosen is already done, during the user rights.
        abs_path = super(AuthenticatedFileHandler, self).validate_absolute_path(os.path.normpath(os.path.join(root, "..", "projects")), absolute_path)
        # End DKU specific
