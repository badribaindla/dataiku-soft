...
class SessionRootHandler(APIHandler):
    ...
    def post(self):
        ...
        self.log.info("Path=%s session_virtual_path=%s" % (path, session_virtual_path))
        # END DKU SPECIFIC

        exists = yield gen.maybe_future(sm.session_exists(path=session_virtual_path))
        if exists:
            model = yield gen.maybe_future(sm.get_session(path=session_virtual_path))
        else:
            try:
...
class SessionHandler(APIHandler):
    ...
    def patch(self, session_id):
        ...
        if 'path' in changes:
            sm = self.session_manager
            session_virtual_path = sm.get_session_virtual_path(path=changes['path'], user=self.get_current_user())
            self.log.info("Patch the patch: origPath=%s session_virtual_path=%s" % (changes['path'], session_virtual_path))
            changes["path"] = session_virtual_path
        # END DKU SPECIFIC
...
_session_id_regex = r"(?P<session_id>[^/&?]+)"
