...
class ContentsHandler(APIHandler):
    ...
    def _execute(self, *args, **kwargs):
        """
        put the DataikuContentsHandler into the RequestContextFactory to be used on the contents manager
        """
        global_data = {'handler': self}
        with StackContext(functools.partial(RequestContextFactory, **global_data)):
            super(ContentsHandler, self)._execute(*args, **kwargs)
    # End DKU specific
...
class CheckpointsHandler(APIHandler):
    ...
    def _execute(self, *args, **kwargs):
        """
        put the CheckpointsHandler into the RequestContextFactory to be used on the contents manager
        """
        global_data = {'handler': self}
        with StackContext(functools.partial(RequestContextFactory, **global_data)):
            super(CheckpointsHandler, self)._execute(*args, **kwargs)
    # End DKU specific
...
class ModifyCheckpointsHandler(APIHandler):
    ...
    def _execute(self, *args, **kwargs):
        ...
        global_data = {'handler': self}
        with StackContext(functools.partial(RequestContextFactory, **global_data)):
            super(ModifyCheckpointsHandler, self)._execute(*args, **kwargs)
    # End DKU specific
