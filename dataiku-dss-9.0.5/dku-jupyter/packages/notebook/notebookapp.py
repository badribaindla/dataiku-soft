...
class NotebookApp(JupyterApp):
    ...
    def init_configurables(self):
        self.log.info("* INIT JUPYTER: %s" % self.runtime_dir)
    ...
    def init_terminals(self):
        # Begin DKU specific: disable terminals when impersonation is enabled
        if "DIP_HOME" in os.environ:
            import json
            fp = os.path.join(os.environ["DIP_HOME"], "config/general-settings.json")
            if os.path.isfile(fp):
                with open(fp) as f:
                    gs = json.load(f)
                    if gs.get("impersonation", {}).get("enabled", False) == True:
                        self.log.info("DSS impersonation is enabled, not enabling Jupyter terminal")
                        return
        try:
            from .terminal import initialize
            initialize(self.web_app, self.notebook_dir, self.connection_url, self.terminado_settings)
            self.log.info("Enabling Jupyter terminal")
