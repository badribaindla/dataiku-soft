...
define([
    ...
    KernelSelector.prototype.new_notebook = function (kernel_name) {
        ...
        that.notebook.contents.new_untitled(parent, {type: "notebook"}).then(
            function (data) {
                // Begin DKU specific: Bind to URL to the DSS page and not the jupyter page.
                var path = utils.encode_uri_components(data.path);
                var notebookName = path.substring(path.lastIndexOf('/') + 1)
                notebookName = notebookName.substring(0, notebookName.lastIndexOf('.'))
                var project = path.substring(0, path.lastIndexOf('/'));
                var url = utils.url_path_join(
                    "/", "projects", project, "notebooks", "jupyter", notebookName
                );
