...
import { ATN, ATNState, ATNStateType, Transition, TransitionType, PredicateTransition, RuleTransition } from 'antlr4ts/atn';
...
export class CodeCompletionCore {
    ...
    public collectCandidates(caretTokenIndex: number, context?: ParserRuleContext): CandidatesCollection {
        ...
        if (this.showResult)
            console.log("States processed: " + this.statesProcessed);  // NOSONAR (protected with 'showResult' flag)


        if (this.showResult) {
            console.log("\n\nCollected rules:\n");  // NOSONAR (protected with 'showResult' flag)
            for (let rule of this.candidates.rules) {
                let path = "";
                for (let token of rule[1]) {
                    path += this.ruleNames[token] + " ";
                }
                console.log(this.ruleNames[rule[0]] + ", path: ", path);  // NOSONAR (protected with 'showResult' flag)
            }
            ...
            for (let symbol of sortedTokens) {
                console.log(symbol);  // NOSONAR (protected with 'showResult' flag)
            }
            console.log("\n\n");  // NOSONAR (protected with 'showResult' flag)
        }
            ...
            if (this.preferredRules.has(ruleStack[i])) {
                ...
                if (addNew) {
                    ...
                    if (this.showDebugOutput)
                        console.log("=====> collected: ", this.ruleNames[i]); // NOSONAR (protected with 'showDebugOutput' flag)
                }
                return true;
            }
        }
    ...
    private processRule(startState: ATNState, tokenIndex: number, callStack: number[], indentation: string): RuleEndStatus {
        ...
        if (!positionMap) {
            ...
            if (positionMap.has(tokenIndex)) {
                if (this.showDebugOutput) {
                    console.log("=====> shortcut"); // NOSONAR (protected with 'showDebugOutput' flag)
                }
                    ...
                    if (!this.translateToRuleIndex(fullPath)) {
                        for (let symbol of set.intervals.toList())
                            if (!this.ignoredTokens.has(symbol)) {
                                if (this.showDebugOutput) {
                                    console.log("=====> collected: ", this.vocabulary.getDisplayName(symbol)); // NOSONAR (protected with 'showDebugOutput' flag)
                                }
                        ...
                        if (set && set.size > 0) {
                            ...
                            if (atCaret) {
                                ...
                                if (set.contains(currentSymbol)) {
                                    if (this.showDebugOutput)
                                        console.log("=====> consumed: ", this.vocabulary.getDisplayName(currentSymbol)); // NOSONAR (protected with 'showDebugOutput' flag)
                                    statePipeline.push({ state: transition.target, tokenIndex: currentEntry.tokenIndex + 1 });
                                }
                            }
                        }
    ...
    private printDescription(currentIndent: string, state: ATNState, baseDescription: string, tokenIndex: number) {
        ...
        if (tokenIndex >= this.tokens.length - 1)
            ...
            output += "<" + this.tokenStartIndex + tokenIndex + "> ";
        console.log(output + "Current state: " + baseDescription + transitionDescription); // NOSONAR (protected with 'showDebugOutput' flag)
    }

    private printRuleState(stack: number[]) {
        if (stack.length == 0) {
            console.log("<empty stack>"); // NOSONAR
            return;
        }

        for (let rule of stack)
            console.log(this.ruleNames[rule]); // NOSONAR
    }
